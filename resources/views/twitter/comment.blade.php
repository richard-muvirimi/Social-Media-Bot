...
<body class="antialiased">
    <div class="container my-3">
        ...
        <h2 class="my-5">Comment a tweet</h2>
        ...
        <div class=" bg-light border rounded-3">
            <div class="m-5">
                <form method="GET" action="{{ url('/comment') }}">
                    ...
                    <div class="form-group">
                        <label for="search" class="m-3"> Comment tweets</label>
                        <input class="form-control m-3" name="search" id="search" value="{{ $search }}"
                            placeholder="Text to search" required>
                        <textarea class="form-control m-3" col="50" row="20" name="comment" id="comment"
                            placeholder="Status to comment">{{ $comment }}</textarea>
                        <small>Will apply to the first tweet only for now, as twitter has a minutely limit on
                            comments.</small>
                    </div>

                    <button class="btn btn-primary m-3" type="submit">Comment</button>
            ...
            @else
                ...
                <div class="m-3">
                    ...
                    @else
                        <div>
                            <p>Commented posts will be listed here.</p>
                        </div>
                    @endif
                </div>
            @endif
